...
class Main_model extends Model {
	...
	function main_list($table_name, $table)
	{
		...
		$this->db->cache_on();
		$tables = "board_".$table;
		//$this->db->select($tables.'.*, users.nickname');
		$this->db->order_by($tables.'.no', 'desc');
		//$this->db->group_by($tables.'.no');
		$this->db->limit(7);
		//$this->db->join('users', 'users.userid='.$tables.'.user_id', 'left');
			...
			$return_val .= '
				<li>
					<div style="float:left;">
						<img src="/images/main/arrow.gif"> <a href="/'.$table.'/view/'.$list['no'].'/page/1"> '.$this->
common->strcut_utf8(strip_tags($list['subject']), 21).' </a>';
    ...
    */
	function comment_list()
	{
		...
		$sql .= "(SELECT 'CI외 질문게시판'    as 'table', 'etc_qna' as 'tbn', no, original_no, contents, user_name, user_id, reg_date FROM board_etc_qna  WHERE  is_delete = 'N' and original_no != '0' order by no desc limit 7 ) ";
		$sql .= "(SELECT 'CI 사이트 소개'    as 'table', 'ci_make' as 'tbn', no, original_no, contents, user_name, user_id, reg_date FROM board_ci_make  WHERE  is_delete = 'N' and original_no != '0' order by no desc limit 7 ) ";
		...
		<ol class="q_latest_ol">
			...
			$return_val .= '
				<li>
					<div style="float:left;">
						<img src="/images/main/arrow.gif"> <a href="/'.$list['tbn'].'/view/'.$list['original_no'].'/page/1#row_num_'.$list['no'].'"> '.$this->
common->strcut_utf8(strip_tags($list['contents']), 21).' </a>';
						/*
						if ($list['reply_count'] > 0) {
							//$return_val .= '&nbsp;<span class="comment">'.$list['reply_count'].'</span>';
						}
    ...
    */
	...
	/**
	* 버그RSS 최근리스트 표시 by CI세상
   	*
   	* @access private : 접근형태
    * @return String $return_val : 문자열
    */
	function bug_rss_list()
	{
		...
		foreach ($data as $item) :
			...
			$return_val .= '
				<li>
					<div style="float:left;">
						<img src="/images/main/arrow.gif"> <a href="'.$item['link'].'" target="_blank"> '.substr($item['title'], 0, 45).' </a>';
					$return_val .= '
					</div>
