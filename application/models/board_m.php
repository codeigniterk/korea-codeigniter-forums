...
class Board_m extends CI_Model {
    ...
    function insert_board($post) //글 작성
	{
        ...
        //첨부파일 DB 입력
		$file_cnt = strip_image_tags_fck($post['contents'], $last_id, '', $this->table, MENU_ID); //글내용, 게시글번호, 타입(있으면 리플, 테이블명, 테이블번호)
	...
	function load_list($page, $rp, $post, $table)
	{
		...
		$this->db->order_by($table.'.reg_date', 'desc');
...
union	(Select MAX(no) as nn from `".$this->table."`  where no < '".$no."' and is_delete = 'N' and original_no ='0')");
	...
	function get_board_list()
	{
		...
		foreach($rs->result() as $row) {
			$board_list[str_replace('board_', '', $row->name_en)] = $row->name;
		}
		return $board_list;
	}
}
