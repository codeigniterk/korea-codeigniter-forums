...
?>
 	<tr class="bg0">
    	<td class="num"><?= $lt['table'] ?></td>
     	<td class="subject"><a href="/<?= $lt['tbn'] ?>/view/<?= $lt['no'] ?>/page/<?=$page_account?>/<?=$search_url?><?=$search_sfl?>" title="<?= $bubble_title ?>"><?= $bl1 ?><?= $this->
common->strcut_utf8(strip_tags($lt['subject']), 30) ?><?= $bl2 ?></a> [<?=$lt['reply_count']?>]<?= $new_icon ?><?= $recom_icon ?></td>
		<td class="name" align="center"><span class='guest'><?= $lt['nickname']; //  if($lt['nickname']) { echo $lt['nickname']; } else { echo $lt['user_name']; } ?></span></td>
    ...
    <div class="board_button">
        ...
        <div style="float:right;"><a href="#" id="write_post"><img src="/images/btn_write.gif" border='0'></a>
		<script>
