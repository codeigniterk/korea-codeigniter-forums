...
function move_board(no) {
	...
	if(cBoard != '<?php echo $this->seg_exp[0]?>') {
		if(confirm('이 게시물을 [' + cBname + '](으)로 이동 합니다.')) {
			location.replace('/<?= $this->seg_exp[0] ?>/move/' + no + '/' + cBoard);
		}
	} else {
...
<strong>
	<span style="font-size:16px;"><?=$views['subject']?></span>   &nbsp;&nbsp;&nbsp;&nbsp;<iframe src="//www.facebook.com/plugins/like.php?href=<?php echo "http://".$_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];?>&send=false&layout=button_count&width=60&show_faces=false&action=like&colorscheme=light&font=arial&height=20" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:90px; height:20px;" allowTransparency="true"></iframe>&nbsp;<g:plusone size="medium"></g:plusone>
	<?php // 관리기능(이동) by 불의회상 110103 ?>
	<? if( $this->session->userdata('auth_code') >= '15' ) : ?>
	<span class="btn_img"><?php echo form_dropdown('move_board_name_en', $board_list, $this->seg_exp[0], "id='move_board_name_en'");?> <span style="cursor:pointer" onclick="move_board('<?=$views['no']?>')">이동</span></span>
	<? endif; ?>
	<?php // 관리기능(이동) 끝?>
</strong>
...
<thead>
<tr>
	<th scope="row"><?=($views['user_name'])?$views['user_name']:$views['user_name']?></th>
...
<div class="board_button">
	...
	<div style="float:right;" id="gBtn7">
		...
		<a href="/<?= $this->seg_exp[0] ?>/edit/<?=$views['no']?>/page/<?=$page_account?>" id="btn_list"><span>&nbsp;&nbsp;수정&nbsp;&nbsp;</span></a>
		<!--a href="#" id="btn_list" class="edit_post"><span>&nbsp;&nbsp;수정&nbsp;&nbsp;</span></a-->
		<script>
		...
		<? } ?>
		<? if ($this->session->userdata('userid') == $views['user_id'] || $this->session->userdata('auth_code') >= '15') { ?><a href="/<?= $this->seg_exp[0] ?>/delete/<?=$views['no']?>/page/<?=$page_account?>" id="btn_list"><span>&nbsp;&nbsp;삭제&nbsp;&nbsp;</span></a><? } ?>
		<? if ($this->session->userdata('userid')) { ?><a href="/<?= $this->seg_exp[0] ?>/write/0" id="btn_list" ><span>&nbsp;&nbsp;글쓰기&nbsp;&nbsp;</span></a><? } ?>
...
common->strcut_utf8(strip_tags($lt['subject']), 30) ?><?= $bl2 ?></a> [<?=$lt['reply_count']?>]<?= $new_icon ?><?= $recom_icon ?></td>
		<td class="name" align="center"><span class='guest'><?= ($lt['user_name'])?$lt['user_name']:$lt['user_name'] ?></span></td>
