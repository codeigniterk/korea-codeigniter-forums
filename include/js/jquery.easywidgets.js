...
//      widget.find(s.selectors.header).append(menuWrap); //<span>태그 추가부분 주석처리
  ...
  function PrepareSortablePlaces(sortableItems, settings){
    ...
    $(s.selectors.places).sortable({
      ...
      stop : function(e, ui){
        ...
        // 2009.6.23 레이아웃의 location 비교하여 이동가능여부 체크
        if($(ui.item).parent().attr("id") == $(ui.item).children().attr("location")){        
        	return true;
        }else{
    	   alert("해당 플러그인은 "+$(ui.item).children().attr('location')+" 에 적용 가능합니다.");
    	   return false;
        }
      }
    });
